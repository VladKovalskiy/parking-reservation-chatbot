"""Smoke test: embeddings + Milvus Lite wiring.

Run:  uv run python scripts/smoke_embeddings.py

Loads the local sentence-transformers embedding model (downloads it on first
run), indexes 3 test documents into a throwaway Milvus Lite database, and
checks that similarity_search returns the expected document. Also verifies
that the multilingual-e5 query:/passage: prefixes are actually sent to the
model during encoding, not just configured and silently ignored.

This is a manual check, not a pytest test — it downloads a large model and
touches a local Milvus Lite file, so it must never run in CI.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        db_path = str(Path(tmp_dir) / "smoke_milvus_lite.db")
        print(f"[milvus] indexing {len(DOCS)} documents into {db_path} ... ", end="", flush=True)
